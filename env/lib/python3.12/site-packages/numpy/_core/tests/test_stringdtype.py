...
from numpy.testing import IS_PYPY, IS_WASM, assert_array_equal
...
def test_ufunc_add(dtype, string_list, other_strings, use_out):
    ...
    result = np.array([a + b for a, b in zip(arr1, arr2, strict=False)], dtype=dtype)
...
def test_ufunc_multiply(dtype, string_list, other, other_dtype, use_out):
    ...
    try:
        len(other)
        result = [s * o for s, o in zip(string_list, other, strict=False)]
...
def test_nat_casts():
    s = 'nat'
    all_nats = itertools.product(*zip(s.upper(), s.lower(), strict=False))
