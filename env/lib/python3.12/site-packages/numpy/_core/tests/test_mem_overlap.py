...
def test_diophantine_fuzz():
    ...
    for ndim in range(10):
        ...
        while min(feasible_count, infeasible_count) < min_count:
            ...
            b_ub = min(max_int-2, sum(a*ub for a, ub in zip(A, U, strict=False)))
            ...
            if X is None:
                ...
                # take too long)
                ranges = tuple(range(0, a*ub+1, a) for a, ub in zip(A, U, strict=False))
                ...
                # Check validity
                assert_(sum(a*x for a, x in zip(A, X, strict=False)) == b)
                assert_(all(0 <= x <= ub for x, ub in zip(X, U, strict=False)))
                feasible_count += 1
...
def iter_random_view_pairs(x, same_steps=True, equal_size=False):
    ...
    while True:
        ...
        s1 = tuple(random_slice(p, s) for p, s in zip(x.shape, steps, strict=False))
        ...
        if equal_size:
            ...
            steps2 = tuple(rng.randint(1, max(2, p//(1+pa)))
                           ...
                           for p, s, pa in zip(x.shape, s1, a.shape, strict=False))
            s2 = tuple(random_slice_fixed_size(p, s, pa)
                       for p, s, pa in zip(x.shape, steps2, a.shape, strict=False))
        elif same_steps:
            steps2 = steps
        else:
        ...
        if not equal_size:
            s2 = tuple(random_slice(p, s) for p, s in zip(x.shape, steps2, strict=False))
...
def test_internal_overlap_diophantine():
    def check(A, U, exists=None):
        ...
        if X is not None:
            assert_(sum(a*x for a, x in zip(A, X, strict=False)) == sum(a*u//2 for a, u in zip(A, U, strict=False)))
            assert_(all(0 <= x <= u for x, u in zip(X, U, strict=False)))
            assert_(any(x != u//2 for x, u in zip(X, U, strict=False)))
...
def check_internal_overlap(a, manual_expected=None):
    ...
    for v in itertools.product(*ranges):
        offset = sum(s*w for s, w in zip(a.strides, v, strict=False))
        if offset in m:
            expected = True
            break
        else:
