...
def test_multiiter_fields(install_temp, arrays):
    ...
    assert all(
        x.base is y.base
        for x, y in zip(bcast.iters, checks.get_multiiter_iters(bcast), strict=False)
    )
...
def test_npyiter_api(install_temp):
    ...
    assert all(
        x is y for x, y in zip(checks.get_npyiter_operands(it), it.operands, strict=False)
    )
    assert all(
        np.allclose(x, y)
        for x, y in zip(checks.get_npyiter_itviews(it), it.itviews, strict=False)
    )
