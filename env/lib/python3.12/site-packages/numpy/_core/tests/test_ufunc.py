...
class TestUfunc:
    ...
    def test_reduceat_shifting_sum(self):
        L = 6
        x = np.arange(L)
        idx = np.array(list(zip(np.arange(L - 2), np.arange(L - 2) + 2, strict=False))).ravel()
    ...
    def test_ufunc_at_inner_loops(self, typecode, ufunc):
        ...
        with warnings.catch_warnings(record=True) as w_loop:
            warnings.simplefilter('always')
            for i, v in zip(indx, vals, strict=False):
    ...
    def test_at_negative_indexes(self, dtype, ufunc):
        ...
        expected = a.copy()
        for i, v in zip(indxs, vals, strict=False):
...
def test_ufunc_types(ufunc):
    ...
    for typ in ufunc.types:
        ...
        if isinstance(res, tuple):
            ...
            for r, t in zip(res, outs, strict=False):
...
def test_ufunc_noncontiguous(ufunc):
    ...
    for typ in ufunc.types:
        ...
        if len(out) == 1:
            ...
            res_n = (res_n,)
            res_o = (res_o,)
        for c_ar, n_ar, o_ar in zip(res_c, res_n, res_o, strict=False):
            dt = c_ar.dtype
