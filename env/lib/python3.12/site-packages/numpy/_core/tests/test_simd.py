...
class _SIMD_BOOL(_Test_Utility):
    ...
    def test_operators_logical(self):
        ...
        data_and = [a & b for a, b in zip(data_a, data_b, strict=False)]
        vand = getattr(self, "and")(vdata_a, vdata_b)
        assert vand == data_and

        data_or = [a | b for a, b in zip(data_a, data_b, strict=False)]
        vor = getattr(self, "or")(vdata_a, vdata_b)
        assert vor == data_or

        data_xor = [a ^ b for a, b in zip(data_a, data_b, strict=False)]
        ...
        data_andc = [(a & ~b) & 0xFF for a, b in zip(data_a, data_b, strict=False)]
        vandc = self.andc(vdata_a, vdata_b)
        assert data_andc == vandc

        data_orc = [(a | ~b) & 0xFF for a, b in zip(data_a, data_b, strict=False)]
        vorc = self.orc(vdata_a, vdata_b)
        assert data_orc == vorc

        data_xnor = [~(a ^ b) & 0xFF for a, b in zip(data_a, data_b, strict=False)]
...
class _SIMD_INT(_Test_Utility):
    ...
    def test_arithmetic_subadd_saturated(self):
        ...
        data_adds = self._int_clip([a + b for a, b in zip(data_a, data_b, strict=False)])
        adds = self.adds(vdata_a, vdata_b)
        assert adds == data_adds

        data_subs = self._int_clip([a - b for a, b in zip(data_a, data_b, strict=False)])
    ...
    def test_math_max_min(self):
        ...
        data_max = [max(a, b) for a, b in zip(data_a, data_b, strict=False)]
        simd_max = self.max(vdata_a, vdata_b)
        assert simd_max == data_max

        data_min = [min(a, b) for a, b in zip(data_a, data_b, strict=False)]
...
class _SIMD_FP(_Test_Utility):
    ...
    def test_arithmetic_fused(self):
        ...
        data_fma = self.load([a * b + c for a, b, c in zip(vdata_a, vdata_b, vdata_c, strict=False)])
    ...
    def test_comparison_with_nan(self, py_comp, np_comp):
        ...
        for case_operand1, case_operand2 in cmp_cases:
            ...
            data_cmp = [py_comp(a, b) for a, b in zip(data_a, data_b, strict=False)]
...
class _SIMD_ALL(_Test_Utility):
        ...
        for stride in range(-64, 64):
            if stride < 0:
                ...
                data_stride = list(itertools.chain(
                    *zip(*[data[-i::stride] for i in range(scale, 0, -1)], strict=False)
                ))
            elif stride == 0:
                data = self._data()
                ...
                data_stride = list(itertools.chain(
                    *zip(*[data[i::stride] for i in range(scale)], strict=False))
                )
    ...
    def test_reorder(self):
        ...
        # zip(interleave)
        data_zipl = self.load([
            v for p in zip(data_a_lo, data_b_lo, strict=False) for v in p
        ])
        data_ziph = self.load([
            v for p in zip(data_a_hi, data_b_hi, strict=False) for v in p
        ])
    ...
    def test_operators_comparison(self, func, intrin):
        ...
        data_cmp = [func(a, b) for a, b in zip(data_a, data_b, strict=False)]
    ...
    def test_operators_logical(self):
        ...
        data_xor = cast_data([a ^ b for a, b in zip(data_cast_a, data_cast_b, strict=False)])
        vxor = cast(self.xor(vdata_a, vdata_b))
        assert vxor == data_xor

        data_or  = cast_data([a | b for a, b in zip(data_cast_a, data_cast_b, strict=False)])
        vor  = cast(getattr(self, "or")(vdata_a, vdata_b))
        assert vor == data_or

        data_and = cast_data([a & b for a, b in zip(data_cast_a, data_cast_b, strict=False)])
        ...
        if self.sfx not in ("u8"):
            return
        data_andc = [a & ~b for a, b in zip(data_cast_a, data_cast_b, strict=False)]
    ...
    def test_arithmetic_subadd(self):
        ...
        # non-saturated
        data_add = self.load([a + b for a, b in zip(data_a, data_b, strict=False)]) # load to cast
        add  = self.add(vdata_a, vdata_b)
        assert add == data_add
        data_sub  = self.load([a - b for a, b in zip(data_a, data_b, strict=False)])
    ...
    def test_arithmetic_mul(self):
        ...
        data_mul = self.load([a * b for a, b in zip(data_a, data_b, strict=False)])
    ...
    def test_arithmetic_div(self):
        ...
        data_div = self.load([a / b for a, b in zip(data_a, data_b, strict=False)])
