...
class FloatingFormat:
    ...
    def fillFormat(self, data):
        ...
        if len(finite_vals) == 0:
            ...
            frac_strs, _, exp_strs = zip(*(s.partition('e') for s in strs), strict=False)
            int_part, frac_part = zip(*(s.split('.') for s in frac_strs), strict=False)
            ...
            strs = (dragon4_positional(x, precision=self.precision,
                                       ...
                                       sign=self.sign == '+')
                    for x in finite_vals)
            int_part, frac_part = zip(*(s.split('.') for s in strs), strict=False)
            if self._legacy <= 113:
...
class StructuredVoidFormat:
    ...
    def __call__(self, x):
        str_fields = [
            ...
            for field, format_function in zip(x, self.format_functions, strict=False)
        ]
        if len(str_fields) == 1:
