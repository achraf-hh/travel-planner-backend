...
def ndenumerate(a, compressed=True):
    ...
    (2, 2) 8
    """
    for it, mask in zip(np.ndenumerate(a), getmaskarray(a).flat, strict=False):
        if not mask:
            yield it
        elif not compressed:
            yield it[0], masked
...
def _ezclump(mask):
    ...
    if mask[0]:
        ...
        r.extend((slice(left, right)
                  for left, right in zip(idx[1:-1:2], idx[2::2], strict=False)))
    else:
        if len(idx) == 0:
            return []

        r = [slice(left, right) for left, right in zip(idx[:-1:2], idx[1::2], strict=False)]
