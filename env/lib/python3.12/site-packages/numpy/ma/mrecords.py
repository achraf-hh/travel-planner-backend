...
def _checknames(descr, names=None):
    ...
    ndescr = []
    for (n, d, t) in zip(new_names, default_names, descr.descr, strict=False):
    ...
    def __str__(self):
        ...
        if self.size > 1:
            mstr = [f"({','.join([str(i) for i in s])})"
                    for s in zip(*[getattr(self, f) for f in self.dtype.names], strict=False)]
            return f"[{', '.join(mstr)}]"
        else:
            mstr = [f"{','.join([str(i) for i in s])}"
                    for s in zip([getattr(self, f) for f in self.dtype.names], strict=False)]
...
def fromarrays(arraylist, dtype=None, shape=None, formats=None,
    ...
    _array._mask.flat = list(zip(*masklist, strict=False))
    if fill_value is not None:
...
def fromtextfile(fname, delimiter=None, commentchar='#', missingchar='',
    ...
    mdescr = list(zip(varnames, vartypes, strict=False))
    ...
    _datalist = [ma.masked_array(a, mask=m, dtype=t, fill_value=f)
                 for (a, m, t, f) in zip(_variables.T, _mask, vartypes, mfillv, strict=False)]
