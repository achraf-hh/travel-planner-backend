...
def piecewise(x, condlist, funclist, *args, **kw):
    ...
    y = zeros_like(x)
    for cond, func in zip(condlist, funclist, strict=False):
...
def select(condlist, choicelist, default=0):
    ...
    condlist = condlist[::-1]
    for choice, cond in zip(choicelist, condlist, strict=False):
...
def trim_zeros(filt, trim='fb', axis=None):
    ...
    if len(start) == 1:
        ...
        # trim all axes
        sl = tuple(slice(*x) for x in zip(start, stop, strict=False))
    else:
...
def _update_dim_sizes(dim_sizes, arg, core_dims):
    ...
    for dim, size in zip(core_dims, core_shape, strict=False):
        if dim in dim_sizes:
...
def _create_arrays(broadcast_shape, dim_sizes, list_of_core_dims, dtypes,
    ...
    if results is None:
        ...
        arrays = tuple(np.empty_like(result, shape=shape, dtype=dtype)
                       ...
                       in zip(results, shapes, dtypes, strict=False))
    return arrays
...
class vectorize:
    ...
    def _call_as_normal(self, *args, **kwargs):
        ...
        if not kwargs and not excluded:
            ...
            def func(*vargs):
                ...
                kwargs.update(zip(names, vargs[len(inds):], strict=False))
        ...
        if self.signature is not None:
            ...
            if ufunc.nout == 1:
                ...
                res = tuple([asanyarray(x, dtype=t)
                             for x, t in zip(outputs, otypes, strict=False)])
        return res
    ...
    def _vectorize_call_with_signature(self, func, args):
        ...
        args = [np.broadcast_to(arg, shape, subok=True)
                for arg, shape in zip(args, input_shapes, strict=False)]
        ...
        for index in np.ndindex(*broadcast_shape):
            ...
            if outputs is None:
                for result, core_dims in zip(results, output_core_dims, strict=False):
                    _update_dim_sizes(dim_sizes, result, core_dims)

                outputs = _create_arrays(broadcast_shape, dim_sizes,
                                         output_core_dims, otypes, results)

            for output, result in zip(outputs, results, strict=False):
                output[index] = result
