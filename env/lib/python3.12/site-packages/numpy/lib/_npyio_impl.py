    ...
    * Custom converters may receive unexpected values due to dtype
    ...
    with fid_ctx:
        ...
        # We have a sequence : each item matches a column
            ...
            for (value, entry) in zip(user_missing_values, missing_values, strict=False):
                value = str(value)
        ...
        if dtype is None:
            ...
            converters = [
                ...
                for (miss, fill) in zip(missing_values, filling_values, strict=False)
            ]
        else:
            ...
            if len(dtype_flat) > 1:
                ...
                zipit = zip(dtype_flat, missing_values, filling_values, strict=False)
                ...
                zipit = zip(missing_values, filling_values, strict=False)
...
#        else:
    ...
    if loose:
        rows = list(
            zip(*[[conv._loose_call(_r) for _r in map(itemgetter(i), rows)]
                  for (i, conv) in enumerate(converters)], strict=False))
    else:
        rows = list(
    ...
    if dtype is None:
        ...
        if names is None:
            ...
            base = {
                c_type
                for c, c_type in zip(converters, column_types, strict=False)
                if c._checked}
            if len(base) == 1:
                uniform_type, = base
            ...
            ddtype = list(zip(names, sized_column_types, strict=False))
            mdtype = list(zip(names, [bool] * len(sized_column_types), strict=False))
    ...
    if usemask and names:
        for (name, conv) in zip(names, converters, strict=False):
