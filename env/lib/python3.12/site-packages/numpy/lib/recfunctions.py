...
def merge_arrays(seqarrays, fill_value=-1, flatten=False,
    ...
    * XXX: I just obtained these values empirically
    ...
    if usemask:
        for (a, n) in zip(seqarrays, sizes, strict=False):
...
def append_fields(base, names, data, dtypes=None,
    ...
    if dtypes is None:
        ...
        data = [a.view([(name, a.dtype)]) for (name, a) in zip(names, data, strict=False)]
    else:
        ...
        data = [np.array(a, copy=None, subok=True, dtype=d).view([(n, d)])
                for (a, n, d) in zip(data, names, dtypes, strict=False)]
    #
...
def _common_stride(offsets, counts, itemsize):
    ...
    if negative:
        ...
        it = zip(reversed(offsets), reversed(counts), strict=False)
    else:
        it = zip(offsets, counts, strict=False)
...
def structured_to_unstructured(arr, dtype=None, copy=False, casting='unsafe'):
    ...
    dts, counts, offsets = zip(*fields, strict=False)
...
def stack_arrays(arrays, defaults=None, usemask=True, asrecarray=False,
    ...
    if len(newdescr) == 1:
        ...
        seen = []
        for (a, n, i, j) in zip(seqarrays, fldnames, offset[:-1], offset[1:], strict=False):
            names = a.dtype.names
            if names is None:
