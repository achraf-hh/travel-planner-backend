...
class TestNanFunctions_MinMax:
    ...
    def test_keepdims(self):
        mat = np.eye(3)
        for nf, rf in zip(self.nanfuncs, self.stdfuncs, strict=False):
...
class TestNanFunctions_ArgminArgmax:
    ...
    def test_result_values(self):
        for f, fcmp in zip(self.nanfuncs, [np.greater, np.less], strict=False):
            for row in _ndat:
...
class TestNanFunctions_SumProd(SharedNanFunctionsTestsMixin):
    ...
    def test_allnans(self, axis, dtype, array):
        ...
        array = array.astype(dtype)
        for func, identity in zip(self.nanfuncs, [0, 1], strict=False):
            out = func(array, axis=axis)
            assert np.all(out == identity)
            assert out.dtype == array.dtype

    def test_empty(self):
        for f, tgt_value in zip([np.nansum, np.nanprod], [0, 1], strict=False):
            mat = np.zeros((0, 3))
            tgt = [tgt_value]*3
            res = f(mat, axis=0)
            assert_equal(res, tgt)
...
class TestNanFunctions_CumSumProd(SharedNanFunctionsTestsMixin):
    ...
    def test_empty(self):
        for f, tgt_value in zip(self.nanfuncs, [0, 1], strict=False):
            mat = np.zeros((0, 3))
    ...
    def test_keepdims(self):
        for f, g in zip(self.nanfuncs, self.stdfuncs, strict=False):
            mat = np.eye(3)
            for axis in [None, 0, 1]:
...
class TestNanFunctions_MeanVarStd(SharedNanFunctionsTestsMixin):
    ...
    def test_ddof(self):
        ...
        for nf, rf in zip(nanfuncs, stdfuncs, strict=False):
            for ddof in [0, 1]:
    ...
    def test_where(self, dtype):
        ...
        for f, f_std in zip(self.nanfuncs, self.stdfuncs, strict=False):
